...
class Template {
    constructor(string) {
        if (typeof (string) === 'string') {
            ...
            const error = new BadParameterError('First parameter of Template() must be a string', string);
    ...
    extract(beginning, ending) {
        ...
        if (beginning < 0) {
            throw new BadParameterError("First parameter of template extract() must be positive");
        }
        if (ending < 0) {
            throw new BadParameterError("Second parameter of template extract() must be positive");
        }
        if (beginning > ending) {
            throw new BadParameterError("Second parameter of template extract() must be bigger than first parameter");
        }
        if (ending > this.content.length) {
            throw new BadParameterError("Second parameter of template extract() must be smaller than the template length");
        }
    ...
    replace(beginning, ending, string) {
        ...
        if (beginning > ending) {
            throw new BadParameterError("Second parameter of template replace() must be bigger than first parameter");
        }
