...
class Template {
    ...
    _searchNextIdentifier() {
        while (this.nextIndex < this.length) {
            if (this.content[this.index] === constant.LINE_BREAK) {
                this.addLine();
            } else {
                const identifier = this.content.substr(this.index, constant.DELIMITER_SIZE);
                if (identifier === constant.GENERIC_OPEN || identifier === constant.GENERIC_CLOSE || identifier === constant.STANDALONE_OPEN || identifier === constant.STANDALONE_CLOSE) {
    ...
    searchNextTag() {
        ...
        if (open.content !== constant.GENERIC_OPEN && open.content !== constant.STANDALONE_OPEN) {
            throw new TemplateError("Tag " + open.content + " found, expected " + constant.GENERIC_OPEN + " or " + constant.STANDALONE_OPEN, this.line);
        }
        const close = this._searchNextIdentifier();
        if (close.content !== constant.GENERIC_CLOSE && close.content !== constant.STANDALONE_CLOSE) {
