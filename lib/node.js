...
class Node {
    ...
    complete(tag, template) {
        ...
        if (tag.isCloseType() === false) {
            throw new LogicError(); //TODO missing text
        }
        ...
        this.computePositions(template);
    ...
    computeRelativePositions(node) {
        if (node) {
            this.relativeStart = this.start - node.getOpenEnd();
            this.relativeEnd = this.end - node.getOpenEnd();
        } else {
    ...
    reset() {
        this._preExecuted = false;
        this._postExecuted = false;
        if (this.hasChildren() === true) { //TODO rework <= why ??
            for (let i = 0; i < this.children.length; i++) {
