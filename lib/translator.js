class Translator {
    ...
    translate(keyword) {
        ...
        if (typeof this.translations[keyword] === "undefined") {
            let error = new Error("Keyword " + keyword + " not found in translations");
        ...
        if (typeof (this.translations[keyword][this.language]) === "undefined" && typeof (this.translations[keyword][this.fallbackLanguage]) === "undefined") {
            let error = new Error('There is no translation for the keyword ' + keyword + ', neither in the language nor in the fallback language');
            error.steUsageFailure = true;
            throw error;
        }
        if (typeof (this.translations[keyword][this.language]) === "undefined" && typeof (this.translations[keyword][this.fallbackLanguage]) !== "undefined") {
    ...
    set translations(translations) {
        if (typeof (translations) !== 'object') {
            let error = new Error('Property translations of translator must be an object, ' + typeof translations + " given");
            error.steUsageFailure = true;
            throw error;
        }
    ...
    set language(language) {
        ...
        if (typeof (language) !== 'string') {
            let error = new Error('Property language of translator must be a string, ' + typeof language + " given");
            error.steUsageFailure = true;
            throw error;
        }
    ...
    set fallbackLanguage(language) {
        if (typeof (language) !== 'string') {
            let error = new Error('Property fallbackLanguage of translator must be a string, ' + typeof language + " given");
            error.steUsageFailure = true;
            throw error;
        }
