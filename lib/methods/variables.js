    ...
    process(template, context) {
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0, length = this._list.length; i < length; i++) {
                console.log("START SAFEEVAL", template, new Date().getTime());
                let variable = safeEval(this._list[i][1], simplifiedContext);
                console.log("END SAFEEVAL", template, new Date().getTime());
                if (!(this._list[i][2])) {
