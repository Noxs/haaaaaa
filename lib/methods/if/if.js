...
const elseRegexp = new RegExp('({%\\s?else\\s?%})\\s?([\\s\\S]+)\\s?{%\\s?endif\\s?%}', 'g');
            ...
            for (let i = 0; i < tags.length; ++i) {
                if (tags[i][1]) {
                    ...
                    if (depth === 0) {
                        ...
                        const elseTag = templateCopy.search(elseRegexp);
                        const IfCondition = require('./ifCondition.js');
                        const ifCondition = new IfCondition(templateCopy, context, tags[0], tags[i], elseTag[0]);
