class ForLoop {
    ...
    process(template, context, openingTag, closingTag) {
        ...
        return new Promise((resolve, reject) => {
            if (typeof this._context.byString(this._openingTag[2]) === "undefined") {
                const error = new Error(this._openingTag[2] + ' is undefined');
                error.steUsageFailure = true;
                return reject(error);
            }
            if (this._context.byString(this._openingTag[2]) === null) {
                const error = new Error(this._openingTag[2] + ' is null');
                error.steUsageFailure = true;
                return reject(error);
            }
            const promises = [];
