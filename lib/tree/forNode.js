...
class ForNode extends Node {
    ...
    getContextForChildren() {
        ...
        if (this._key !== null) {
            context[this._key] = this._currentIteration;
        }
        context[this._value] = this.context.byString(this._forContextVariableName)[this._currentIteration];
        return context;
    }
    ...
    _parseExpression() {
        ...
        if (this._forContextVariableName === "") {
            throw new TemplateError("Invalid '" + forText + "' expression", this.open.line);
        }
    ...
    postExecute() {
        if (this._iterationNumber !== 0) {
            ...
            this._results.push(iterationResult);
        }

        this.postExecutionDone();
        if (this._currentIteration < this._iterationNumber) {
            this.reset();
            return this;
        } else {
            this.result = this._results.join("");
            this._init();
            if (this.hasNext()) {
                return this.getNext();
