...
class Tree {
    ...
    create() {// rename parse or build?
        for (let tag = this.template.searchNextTag(); tag !== null; tag = this.template.searchNextTag()) { // TODO access private property here? better while not the end
            if (tag.isOpenType() === true) {
                ...
                this._currentNode.complete(tag, this.template); //ERROR MANAGEMENT ??????
        ...
        if (this.nodeFactory.isOnFloor() === false) {
            throw new Error("One tag is missing");/* ONE TAGE IS MISSING???? WITCH ONE??? LINE??? ERROR TYPE???*/
        }
        this._built = true;
        return this;
    }
    ...
    _getFirstNode() {
        if (this.isBuilt() === false) {
            throw new Error({ level: "developper", type: "bad use", message: "You must call create() before execute()" });//not realy nice (method name) // replace with error
        } else {
            return this.start;
        }
    }
    ...
    execute(context) {
        ...
        ///render here browse tree and replace with result
        // here or in a another function 
        // loop on nodes but start a the end and go back to start
        return this;
    }
