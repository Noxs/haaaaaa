...
class ArrayFilterNode extends FilterNode {
    ...
    _parse() {
        ...
        if (this._rawFilter[0] !== constant.SQUARE_BRACKET_OPEN) {
            throw new TemplateError("Expected " + constant.SQUARE_BRACKET_OPEN + " but found " + this._rawFilter[0], this._line);
        }

        if (this._rawFilter[this._rawFilter.length - 1] !== constant.SQUARE_BRACKET_CLOSE) {
            throw new TemplateError("Expected " + constant.SQUARE_BRACKET_CLOSE + " but found " + this._rawFilter[this._rawFilter.length - 1], this._line);
        }
        ...
        for (let i = 0, length = this._rawFilter.length; i < length; i++) {
            ...
            } else if (this._rawFilter[i] === constant.COMMA && singleQuote === false && doubleQuote === false && squareBracket === 0 && curlyBracket === 0 && bracket === 0) {
