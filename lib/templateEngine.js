...
class TemplateEngine {
    ...
    render(template, context) {
        return new Promise((resolve, reject) => {
            ...
            console.log("START FOR", random, new Date().getTime());
            forRepetition.process(template, context).then(() => {
                console.log("END FOR", random, new Date().getTime());
                console.log("START IF", random, new Date().getTime());
                ifCondition.process(template, context).then(() => {
                    console.log("END IF", random, new Date().getTime());
                    console.log("START VAR", random, new Date().getTime());
                    this._variables.process(template, context).then(() => {
                        console.log("END VAR", random, new Date().getTime());
                        resolve(template);
                    }, (error) => {
