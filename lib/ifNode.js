...
class IfNode extends Node {
    ...
    _getExpressionToEvaluate(tag) {
        if (tag.isIfCategory() === true) {
            const expression = tag.content.substr(keywords.IF.length).trim();
            if (expression.length === 0) {
                throw new TemplateError("Invalid '" + keywords.IF + "' expression", this.open.line);
            }
            return expression;
        } else if (tag.isElseCategory() === true) {
            return "";
        } else if (tag.isElseifCategory() === true) {
            const expression = tag.content.substr(keywords.ELSEIF.length).trim();
            if (expression.length === 0) {
                throw new TemplateError("Invalid '" + keywords.ELSEIF + "' expression", this.open.line);
            }
            return expression;
        }
    }
