    ...
    _parse() {
        if (this.getDepth() === 0 && this._rawFilter.indexOf("(") === -1) {
            ...
            if (this._functionName.length === 0) {
                throw new TemplateError("Expected function name before " + constant.BRACKET_OPEN, this._line);
            }
            ...
            for (let length = this._value.length; i < length; i++) {
                if (this._value[i] === constant.CURLY_BRACKET_OPEN && singleQuote === false && doubleQuote === false) {
                    curlyBracket++;
                } else if (this._value[i] === constant.CURLY_BRACKET_CLOSE && singleQuote === false && doubleQuote === false) {
                    curlyBracket--;
                } else if (this._value[i] === constant.SQUARE_BRACKET_OPEN && singleQuote === false && doubleQuote === false) {
                    squareBracket++;
                } else if (this._value[i] === constant.SQUARE_BRACKET_CLOSE && singleQuote === false && doubleQuote === false) {
                    squareBracket--;
                } else if (this._value[i] === constant.BRACKET_OPEN && singleQuote === false && doubleQuote === false) {
                    bracket++;
                } else if (this._value[i] === constant.BRACKET_CLOSE && singleQuote === false && doubleQuote === false) {
                    bracket--;
                }

                if (this._value[i] === constant.SINGLE_QUOTE && doubleQuote === false) {
                    singleQuote = !singleQuote;
                } else if (this._value[i] === constant.DOUBLE_QUOTE && singleQuote === false) {
                    doubleQuote = !doubleQuote;
                } else if (this._value[i] === constant.COMMA && singleQuote === false && doubleQuote === false && squareBracket === 0 && curlyBracket === 0 && bracket === 0) {
