    ...
    stringify(expression) {
        if (this._stringify === false) {
            ...
            while (this._modify.length !== 0) {
                ...
                this._outputString = this._outputString.substr(0, this._property[key].start) + string + this._outputString.substr(this._property[key].end + 1);
                ...
                for (let current in this._property) {
                    if (position < current.position) {
                        //TODO this is not testable at the moment
