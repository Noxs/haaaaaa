class Filters {
    constructor(templateEngine) {
        this._regexp = new RegExp("([a-zA-Z0-9.[\\]]+)(?:\\((.+?)\\))", "g");
        this._templateEngine = templateEngine;
        this.add({ process: require('../filters/translate.js'), name: 'translate' });
        this.add({ process: require('../filters/size.js'), name: 'size' });
        this.add({ process: require('../filters/date.js'), name: 'date' });
    }
    ...
    applyFilter(tag, variable, context) {
        return new Promise((resolve, reject) => {
            ...
            if (tag[3]) {
                ...
                const nestedFilters = parameters.search(this._regexp);
