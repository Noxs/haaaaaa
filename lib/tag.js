...
class Tag {
    ...
    _create() {
        if (this.content.startsWith(constant.FOR_KEYWORD + constant.SPACER) === true) {
            this._setCategoryToFor();
            this._setTypeToOpen();
        } else if (this.content.startsWith(constant.ENDFOR_KEYWORD) === true) {
            this._setCategoryToFor();
            this._setTypeToClose();
        } else if (this.content.startsWith(constant.ELSEIF_KEYWORD + constant.SPACER) === true) {
            this._setCategoryToElseif();
            this._setTypeToMiddle();
        } else if (this.content.startsWith(constant.IF_KEYWORD + constant.SPACER) === true) {
            this._setCategoryToIf();
            this._setTypeToOpen();
        } else if (this.content.startsWith(constant.ENDIF_KEYWORD) === true) {
            this._setCategoryToIf();
            this._setTypeToClose();
        } else if (this.content.startsWith(constant.STYLE_KEYWORD) === true) {
            this._setCategoryToStyle();
            this._setTypeToStandalone();
        } else if (this.content.startsWith(constant.ELSE_KEYWORD) === true) {
    ...
    get content() {
        if (this._content === null) {
            this._content = this._rawContent.substr(constant.DELIMITER_SIZE, this._rawContent.length - (constant.DELIMITER_SIZE * 2));
            let i = 0;
